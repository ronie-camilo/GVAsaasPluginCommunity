...
namespace Gvinci.Plugin.Action
{
    internal class GPluginActionAsaasCustomerCreate : GPluginAction
    {
        ...
        public override string Name => "Incluir cliente";
        ...
        public GPluginActionAsaasCustomerCreate(IGPlugin Plugin) : base(Plugin)
        {
            ...
            {
                new GPluginActionParameter() {ID = 1, Name = "Subação (Novo|Listar|Atualizar|Remover|Restaurar)", Type = PluginActionParameterTypeEnum.STRING},
                new GPluginActionParameter() {ID = 2, Name = "Token Asaas", Type = PluginActionParameterTypeEnum.STRING },
                ...
                new GPluginActionParameter() {ID = 20, Name = "Ambiente (S=Sandbox e P=Produção)", Type = PluginActionParameterTypeEnum.STRING },
                new GPluginActionParameter() {ID = 21, Name = "Retorno - ID do cliente", Type = PluginActionParameterTypeEnum.CONTROL, AllowedControlTypes = new string[] {"GTEXTBOX"} },
                new GPluginActionParameter() {ID = 22, Name = "Retorno - Conteudo do retorno da API", Type = PluginActionParameterTypeEnum.CONTROL, AllowedControlTypes = new string[] {"GTEXTBOX"} },
           };
       }

        public override void WriteActionCall(StringBuilder Builder, int Identation, int ActionSequence)
        {
            string Subaction = this.Parameters[0].Value.ToString();
            string Token = this.Parameters[1].Value.ToString();
            ...
            string Ambiente = this.Parameters[19].Value.ToString();

            IGPluginControl CustomerID = this.Parameters[20].Value as IGPluginControl;
            ...
            //Builder.AppendLine(indentStr + $"var response = GvinciAsaasCommunity.Asaas_CustomerCreateOrUpdate({Token}, {Nome}, {CpfCnpj}, {Email}, {Telefone}, {Celular}, {Cep}, {Numero}, {Referencia}, {Ambiente});");
